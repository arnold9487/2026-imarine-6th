...
def main():
    ...
    # 統計只取資料完整區間（2025 全年）；頭尾多取的月份僅供掃描線算跨年在泊、不進統計
    from datetime import date
    STAT_LO, STAT_HI = date(2025, 1, 1), date(2025, 12, 31)
    stat_idx = [i for i in range(ndays) if STAT_LO <= (dmin + timedelta(days=i)) <= STAT_HI]

    terminals_out = {}
    day_max_per_ct = {}
    for ct, (name, berths) in TERMINALS.items():
        full_series = day_max_series(by_ct.get(ct, []))    # 全資料範圍(掃描線需完整才能算跨年在泊)
        series = [full_series[i] for i in stat_idx]         # 只留 2025 全年做統計/繪圖
        ...
        # ---- 壓力上限：營運日(排除零日)第 95 百分位；不假設分布，避免 Poisson 爭議 ----
        if nonzero:
            limit = max(1, int(math.ceil(np.percentile(nonzero, 95))))
            pct99 = max(1, int(math.ceil(np.percentile(nonzero, 99))))
            method = "營運日第95百分位"
        else:
            limit = 1; pct99 = 1; method = "n/a"
        terminals_out[ct] = {
            "name_zh": name, "berths": berths, "limit": limit, "limit_method": method,
            "pct95": limit, "pct99": pct99,
            "daily_mean_nz": round(mean, 3), "daily_std_nz": round(std, 3),
            "daily_var_nz": round(var, 3), "var_over_mean": round(ratio, 3),
            "nonzero_days": len(nonzero), "total_days": len(series),
    ...
    daymax_dump["_range"] = [STAT_LO.isoformat(), STAT_HI.isoformat()]
