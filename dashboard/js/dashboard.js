...
// ---- 淺色/列印省墨主題:網址 ?light、按 L 切換、列印時自動套用 ----
const setLight = on => document.body.classList.toggle("light", on);
if (/(?:^|[?&#])light(?:=|&|$)/.test(location.search + location.hash)) setLight(true);
document.addEventListener("keydown", e => {
  if (e.key !== "l" && e.key !== "L") return;
  if (e.target instanceof Element && e.target.closest("input,select,textarea")) return;
  setLight(!document.body.classList.contains("light"));
});
let lightBeforePrint = false;
addEventListener("beforeprint", () => {
  lightBeforePrint = document.body.classList.contains("light");
  setLight(true);
});
addEventListener("afterprint", () => setLight(lightBeforePrint));
