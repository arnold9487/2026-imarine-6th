...
async function init() {
  // 離線單機版：資料改由 data/zones.js、data/port_data.js 以 <script> 預先載入為全域變數，
  // 不再使用 fetch（file:// 直接雙擊開啟時 fetch 會被瀏覽器擋掉）。
  const zones = window.ZONES_DATA;
  const data = window.PORT_DATA;
...
function enterCT(ct, x, y) {
  ...
  // 只有「首次進入某個 CT」時才定位懸浮框；之後滑鼠在同一 CT 內移動不再跟著跑
  if (S.activeCT !== ct) {
    S.activeCT = ct; buildLens(ct); fillTooltip(ct);
